...
class InternalMonologue:
    ...
    def process_monologue_tick(self, force_thought_generation=False):
        ...
        if (datetime.now() - last_monologue_time > timedelta(minutes=MONOLOGUE_COOLDOWN_MINUTES)) or force_thought_generation:
            ...
            attention_manager.update_focus("last_monologue_time", datetime.now().isoformat(), salience=0.1)
    ...
    def _generate_thought(self, topic: Optional[str] = None):
        ...
        recent_internal_thoughts_summary = ", ".join(recent_internal_thoughts[:3])
        ...
        if topic:
            ...
            if current_emotion.get("pleasure", 0) < -0.4:
                ...
                prompt = self._build_prompt(current_focus, recent_internal_thoughts_summary)
                if not prompt:
        ...
        if thought:
            ...
            self.thought_logger.info(thought)
            ...
            try:
                ...
                memory_manager.add_event("internal_thought", event_data)
            ...
            attention_manager.update_focus("internal_thoughts", updated_thoughts[:5], salience=0.2, expiry_seconds=3600)
    ...
    def _think_about_accomplishment(self) -> str:
        ...
        if not recent_accomplishments:
            return self._generate_self_compassion_thought()
    ...
    def _build_prompt(self, focus: dict, recent_internal_thoughts_summary: str) -> str:
        templates = []
        ...
        if meta_desire_data:
            ...
            templates.append(meta_template)
            templates.append(meta_template)
