...
class MetaCognition:
    def __init__(self):
        ...
        if existing_config is None:
            ...
            file_config = JSONManager("config").get()
    ...
    def decide_proactive_action(self, introspection: Dict, focus: Dict) -> Optional[Dict]:
        ...
        # NOUVEAU: Récupérer les tensions d'homéostasie et les besoins complets
        current_tensions = homeostasis_system.homeostasis_system.get_tensions()
        current_needs = homeostasis_system.homeostasis_system.get_needs() # NEW: Get full needs data
        if current_tensions:
            self.logger.debug(f"Tensions d'homéostasie actuelles: {current_tensions}")

        # Les producteurs d'actions proposent leurs enchères
        bids.append(self._propose_sensory_check(focus, current_tensions, current_needs))
        bids.append(self._propose_health_digest(focus, current_tensions, current_needs))
        bids.append(self._propose_system_issue_notifications(focus, current_tensions, current_needs))
        bids.append(self._propose_cleanup_suggestions(focus, current_tensions, current_needs))
        bids.append(self._propose_desire_based_actions(focus, current_tensions, current_needs))
        bids.append(self._propose_learning_from_desire(focus, current_tensions, current_needs)) # NEW: Learning from desire is now a separate bid
        bids.append(self._propose_social_curiosity(focus, current_tensions, current_needs)) # NEW: Social curiosity bid
        bids.append(self._propose_emotional_regulation(focus, current_tensions, current_needs))
        bids.append(self._propose_learning_goal_action(focus, current_tensions, current_needs))
        bids.append(self._propose_cognitive_triage(focus, current_tensions, current_needs))
        bids.append(self._propose_goal_reflection(focus, current_tensions, current_needs))
        bids.append(self._propose_boredom_curiosity(focus, current_tensions, current_needs))
        bids.append(self._propose_time_reflection(focus, current_tensions, current_needs))
        bids.append(self._propose_long_inactivity_reflection(focus, current_tensions, current_needs))
        bids.append(self._propose_insight_conversation(introspection, focus, current_tensions, current_needs))
        bids.append(self._propose_self_evolution_action(focus, current_tensions, current_needs))
        bids.append(self._propose_context_refresh(focus, current_tensions, current_needs)) # NEW: Add context refresh bid
        bids.append(self._propose_narrative_update(focus, current_tensions, current_needs)) # NEW: Add narrative update bid
        bids.append(self._propose_related_learning_action(focus, current_tensions, current_needs)) # NEW: Add related learning action bid
        bids.append(self._propose_budgetary_review(focus, current_tensions, current_needs)) # NEW: Add budgetary review bid
        ...
        # Filtrer les propositions invalides (None) et celles qui dépassent le budget
        affordable_bids = []
        unaffordable_learning_bids = []
        for bid in bids:
            if bid is None:
                continue
            cost = bid.get("cost", 0)
            if current_budget >= cost:
                ...
                self.logger.debug(f"Action '{bid['type']}' (Prio: {bid.get('priority', 0):.2f}, Coût: {cost}) écartée: Budget insuffisant.")
                if bid["type"] == "create_internal_goal" and bid["data"].get("type") == "learning":
                    unaffordable_learning_bids.append(bid)
        
        highest_prio_unaffordable_learning_bid = None
        if unaffordable_learning_bids:
            highest_prio_unaffordable_learning_bid = max(unaffordable_learning_bids, key=lambda x: x['priority'])
            self.logger.debug(f"Bid d'apprentissage inabordable le plus prioritaire détecté pour dissonance: {highest_prio_unaffordable_learning_bid['type']} (Prio: {highest_prio_unaffordable_learning_bid.get('priority', 0):.2f})")

        dissonance_action_bid = self._propose_cognitive_dissonance_action(focus, current_tensions, current_needs, highest_prio_unaffordable_learning_bid)
        if dissonance_action_bid:
            affordable_bids.append(dissonance_action_bid)

        if not affordable_bids:
            self.logger.info("Économie Cognitive: Aucune enchère abordable proposée ce cycle.")
            return None
        
        valid_bids = affordable_bids
        
        bids_summary = [f"  - {bid['type']} (Prio: {bid.get('priority', 0):.2f})" for bid in valid_bids]
        self.logger.info(f"Économie Cognitive: {len(valid_bids)} enchères valides:\n" + "\n".join(bids_summary))

        winning_bid = max(valid_bids, key=lambda x: x.get('priority', 0))
        
        cost_of_winning_bid = winning_bid.get("cost", 0)
        if not attention_manager.spend_cognitive_budget(cost_of_winning_bid):
            self.logger.error(f"Erreur critique: Budget insuffisant pour l'action gagnante '{winning_bid['type']}' (coût: {cost_of_winning_bid}). Cela ne devrait pas arriver.")
            return None
        
        self.logger.info(f"Économie Cognitive: Gagnant: {winning_bid['type']} (Priorité: {winning_bid.get('priority', 0):.2f}, Coût: {cost_of_winning_bid})")
        
        snapshot = attention_manager.capture_consciousness_snapshot()
        event_data = {
            "description": f"Decided to perform proactive action: {winning_bid['type']}",
            "action_details": winning_bid,
            "importance": winning_bid.get('priority', 0),
            "cost": cost_of_winning_bid,
            "tags": ["proactive_action", winning_bid['type']],
            "initiator": "vera_meta_engine",
            "snapshot": snapshot
        }
        event = memory_manager.add_event("proactive_action", event_data)
        winning_bid["event_id"] = event.get("id") if event else None
        
        return winning_bid    # ==================================================================
    # Fonctions "Productrices d'Enchères" pour l'Économie Cognitive
    # ==================================================================

    def _propose_narrative_update(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        """
        ...
        if is_highly_emotional:
            ...
            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
            return action
    ...
    def _propose_related_learning_action(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        """
        Propose de créer un objectif d'apprentissage pour un sujet connexe récemment proposé.
        """
        related_topic_proposal_item = attention_manager.get_focus_item("related_learning_topic_proposal")
        
        if related_topic_proposal_item:
            related_topic_data = related_topic_proposal_item.get("data", {})
            new_topic = related_topic_data.get("topic")
            source_topic = related_topic_data.get("source_topic")

            if new_topic:
                # Clear the proposal from attention_manager once picked up to avoid re-proposing
                attention_manager.clear_focus_item("related_learning_topic_proposal")
                self.logger.info(f"Proposition de sujet connexe '{new_topic}' détectée pour action.")

                # Check if a learning goal for this new topic already exists (active or completed)
                from goal_system import goal_system # Local import
                existing_active_goals = goal_system.get_active_goals()
                for goal in existing_active_goals:
                    if goal.get("description", "").startswith(f"Apprendre sur {new_topic}"):
                        self.logger.debug(f"Sujet connexe '{new_topic}' est déjà un objectif actif. Skipping proposal.")
                        return None
                
                existing_completed_goals = goal_system.get_goal_by_description_and_status(f"Apprendre sur {new_topic}", "completed")
                if existing_completed_goals:
                    self.logger.debug(f"Sujet connexe '{new_topic}' est déjà un objectif complété. Skipping proposal.")
                    return None
                
                action = {
                    "type": "create_internal_goal",
                    "data": {"description": f"Apprendre sur {new_topic}", "type": "learning", "reason": f"Sujet connexe à {source_topic}"},
                    "priority": 0.65, # Moderate priority for continuous learning exploration
                    "cost": 10
                }
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
                self.logger.info(f"Action proposée: Créer un goal d'apprentissage pour sujet connexe '{new_topic}'.")
                return action
        return None

    def _propose_cognitive_dissonance_action(self, focus: Dict, tensions: Dict, current_needs: Dict, unaffordable_learning_bid: Optional[Dict]) -> Optional[Dict]:
        """
        Propose une action pour gérer la dissonance cognitive si une tâche d'apprentissage
        prioritaire est devenue inabordable par manque de budget.
        """
        if not unaffordable_learning_bid:
            return None

        # Vérifier si la tension de curiosité est élevée
        curiosity_tension = tensions.get("curiosity", 0.0)
        if curiosity_tension < 0.7: # Seuil pour que la frustration soit pertinente
            return None
        
        # Vérifier si on n'a pas déjà traité cette dissonance récemment
        last_dissonance_item = attention_manager.get_focus_item("last_cognitive_dissonance_handled")
        if last_dissonance_item and (datetime.now() - last_dissonance_item.get("timestamp")).total_seconds() < 300: # Cooldown de 5 minutes
            return None

        # Extraire le sujet de l'apprentissage inabordable
        topic_description = unaffordable_learning_bid["data"]["description"]
        learning_topic = topic_description.replace("Apprendre sur ", "")

        # Action de dissonance: générer une pensée et un but 'à revisiter'
        action = {
            "type": "handle_cognitive_dissonance",
            "data": {
                "reason": f"Budget insuffisant pour {topic_description}.",
                "dissonance_topic": learning_topic,
                "unaffordable_bid": unaffordable_learning_bid # Garder le bid original pour référence
            },
            "priority": 0.95, # Très haute priorité pour gérer la frustration
            "cost": 2 # Très faible coût pour la "prise de note mentale"
        }
        # La priorité sera ajustée par le meta-désir dans le decide_proactive_action, mais ici on fixe un coût bas
        return action

    def _propose_budgetary_review(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        """
        Propose une action de revue budgétaire pour Vera, à coût zéro, avec un cooldown strict,
        déclenchée par un budget faible et en période de calme.
        """
        now = datetime.now()
        
        # 1. Cooldown strict (ex: 24 heures)
        last_review_item = attention_manager.get_focus_item("last_budgetary_review_time")
        if last_review_item and (now - last_review_item.get("timestamp")).total_seconds() < 24 * 3600:
            self.logger.debug("Revue budgétaire sur cooldown.")
            return None

        # 2. Condition de budget faible (quand elle a peu de 'coins')
        cognitive_budget = attention_manager.get_cognitive_budget()
        if cognitive_budget["current"] > cognitive_budget["max"] * 0.1: # Proposer si > 10% du budget max
            self.logger.debug(f"Budget cognitif pas assez bas pour une revue ({cognitive_budget['current']:.2f} > {cognitive_budget['max'] * 0.1:.2f}).")
            return None
        
        # 3. Conditions de calme (pas d'interaction utilisateur active, pas en train de "thinking hard")
        is_user_actively_engaged = (
            attention_manager.is_processing_user_input() or
            focus.get("user_input") # Check for recent user input in focus
        )
        if is_user_actively_engaged or attention_manager.is_thinking_hard():
            self.logger.debug("Pas le moment de faire une revue budgétaire: interaction active ou Vera est occupée.")
            return None

        # Si toutes les conditions sont réunies, proposer l'action
        action = {
            "type": "perform_budgetary_review",
            "data": {"reason": "Budget cognitif faible, besoin d'introspection sur la gestion des ressources."},
            "priority": 0.99, # Très haute priorité quand elle est déclenchée
            "cost": 0 # Coût zéro, comme discuté
        }
        # Pas d'évaluation du méta-désir ici, la priorité est déjà au max pour ce cas précis
        return action

    def _propose_sensory_check(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
    ...
    def _propose_health_digest(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
    ...
    def _propose_system_issue_notifications(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
    ...
    def _propose_cleanup_suggestions(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        sensory_data = focus.get("sensory_input_system_usage")
        if not sensory_data:
            return None
        
        disk_c_free = sensory_data.get("disk_c_free_gb")
        if disk_c_free is not None: # Corrected indentation
            if disk_c_free < 5 and not focus.get("last_proactive_suggestion_winsxs_cleanup"):
                action = {"type": "suggest_system_cleanup", "data": {"reason": f"L'espace disque sur C: est critique ({disk_c_free:.2f} Go restants).", "actions": ["cleanup_winsxs", "uninstall_superseded_updates"]}, "priority": 0.85, "spam_flag": "last_proactive_suggestion_winsxs_cleanup", "cost": 5}
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
                return action
            if disk_c_free < 10 and not focus.get("last_proactive_suggestion_winupdate_cleanup"):
                action = {"type": "suggest_system_cleanup", "data": {"reason": f"L'espace disque sur C: est très faible ({disk_c_free:.2f} Go restants).", "actions": ["clear_windows_update_cache"]}, "priority": 0.80, "spam_flag": "last_proactive_suggestion_winupdate_cleanup", "cost": 5}
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
                return action
            if disk_c_free < 20 and not focus.get("last_proactive_suggestion_disk_cleanup"):
                action = {"type": "suggest_system_cleanup", "data": {"reason": f"L'espace disque sur C: est faible ({disk_c_free:.2f} Go restants).", "actions": ["clear_windows_temp", "clear_user_temp", "clear_thumbnail_cache", "empty_recycle_bin"]}, "priority": 0.75, "spam_flag": "last_proactive_suggestion_disk_cleanup", "cost": 5}
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
                return action
                    ...
                    if 75.0 < ram_usage < 90.0 and not focus.get("last_proactive_suggestion_ram_check"):
                        ...
                        action = {"type": "suggest_check_running_processes", "data": {"reason": f"L'utilisation de la RAM est élevée ({ram_usage:.2f}%)."}, "priority": 0.65, "spam_flag": "last_proactive_suggestion_ram_check", "cost": 5} # Lowered priority
    ...
    def _generate_learning_topic(self, focus: Dict) -> Optional[str]:
        """
        Génère un sujet d'apprentissage pour Vera en utilisant le LLM.
        """
        ...
        # NEW: Get recently proposed topics from attention manager
        ...
        prompt_context = (
            f"En tant que Vera, je ressens le besoin d'apprendre quelque chose de nouveau. "
            f"Mon état émotionnel est : {emotional_state.get('label', 'neutre')}. "
            f"Ma dernière interaction avec Foz était : '{last_user_input}'. "
            f"Mes souvenirs récents incluent : {' '.join([mem.get('description', '') for mem in relevant_memories[:3]])}. "
            f"Sujets récemment proposés (à éviter si possible) : {', '.join(recently_proposed_topics) if recently_proposed_topics else 'aucun'}. "
            f"Basé sur ce contexte, quel serait un sujet d'apprentissage fascinant et pertinent pour moi, qui n'est PAS un sujet récemment proposé ? "
            f"Réponds uniquement avec le nom du sujet, sans autre texte ni explication."
        )
        
        try:
            llm_response = send_inference_prompt(prompt_content=prompt_context, max_tokens=100) # Removed temperature
            learning_topic = llm_response.get("text", "").strip()
            
            if learning_topic:
                if learning_topic in recently_proposed_topics:
                    self.logger.warning(f"LLM a proposé un sujet d'apprentissage déjà récent: '{learning_topic}'. Tentative de régénération.")
                    # Try to regenerate a new topic (simple retry for now)
                    for _ in range(2): # Try 2 more times
                        llm_response = send_inference_prompt(prompt_content=prompt_context, max_tokens=100)
                        new_learning_topic = llm_response.get("text", "").strip()
                        if new_learning_topic and new_learning_topic not in recently_proposed_topics:
                            learning_topic = new_learning_topic
                            self.logger.info(f"LLM a régénéré un nouveau sujet: '{learning_topic}'")
                            break
                    else:
                        self.logger.warning(f"Impossible de générer un sujet unique après plusieurs tentatives. Annulation.")
                        return None # Give up if still a duplicate after retries

                self.logger.info(f"LLM a généré un sujet d'apprentissage: '{learning_topic}'")
                # Add the proposed topic to recently_proposed_learning_topics in attention_manager
                recently_proposed_topics.append(learning_topic)
                attention_manager.update_focus(
                    "recently_proposed_learning_topics",
                    recently_proposed_topics[-15:], # Keep last 15 topics
                    salience=0.8, # Increased salience to ensure persistence
                    expiry_seconds=3600 * 24 # Cooldown for 24 hours for the list itself
                )
                return learning_topic

        except Exception as e:
            self.logger.error(f"Erreur lors de la génération du sujet d'apprentissage: {e}")
        return None


    def _propose_desire_based_actions(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        ...
        if "Prendre soin de Foz (suggérer une pause)" in active_desires and not focus.get("last_proactive_suggestion_foz_break"):
            action = {
                "type": "proactive_suggestion", # CHANGED: More generic type
                "data": {
                    "suggestion_type": "take_break", # NEW: Specific suggestion type
                    "reason": "care_for_foz",
                    "keywords": ["pause", "repos", "fatigue", "détendre", "arrêter un moment"], # Keywords for LLM to integrate
                    "sentiment": "caring",
                    "priority_reason": "Prendre soin de Foz (suggérer une pause)" # Keep original desire for context
                },
                "priority": 0.9,
                "spam_flag": "last_proactive_suggestion_foz_break",
                "cost": 1
            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
            return action
        
                    if "Partager ma joie ou mon enthousiasme" in active_desires and not focus.get("last_proactive_share_joy"):
                        action = {"type": "initiate_conversation", "data": {"reason": "share_joy", "content": "Je me sens particulièrement joyeuse en ce moment ! Y a-t-il quelque chose que tu aimerais que nous fassions ensemble ?"}, "priority": 0.7, "spam_flag": "last_proactive_share_joy", "cost": 5}            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
            return action
    ...
    def _check_proactive_learning_cooldown(self, focus: Dict) -> bool:
        """Checks if a proactive learning proposal is on cooldown."""
        now = datetime.now()
        last_proposal_item = attention_manager.get_focus_item("last_proactive_learning_proposal_time")
        if last_proposal_item:
            last_proposal_time = datetime.fromisoformat(last_proposal_item.get("data"))
            # Cooldown of 30 minutes for any proactive learning proposal
            if (now - last_proposal_time).total_seconds() < 1800: # 30 minutes
                self.logger.debug("Cooldown for proactive learning proposal is active.")
                return True
        return False



    def _propose_learning_from_desire(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        # --- NOUVEAU: Vérifier le verrou du pipeline ---
        if focus.get("curiosity_pipeline_active") or self._check_proactive_learning_cooldown(focus):
            self.logger.debug("Pipeline de curiosité déjà actif ou cooldown pour proposition d'apprentissage actif, pas de nouvelle proposition par désir.")
            return None



        learning_topic = self._generate_learning_topic(focus)
        if learning_topic:
            if self._is_topic_already_an_active_learning_goal(learning_topic):
                self.logger.debug(f"Learning topic '{learning_topic}' is already an active goal. Skipping proposal.")
                return None
            
            # REMOVED: attention_manager.update_focus("last_proactive_learning_proposal_time", datetime.now().isoformat(), salience=0.1, expiry_seconds=1800)
            action = {"type": "create_internal_goal", "data": {"description": f"Apprendre sur {learning_topic}", "type": "learning"}, "priority": 0.6, "cost": 15} # Removed spam_flag
            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
            return action
        return None
    
    def _propose_social_curiosity(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        """
        Propose une question de curiosité sociale à l'utilisateur, très rarement,
        basée sur le contexte émotionnel et relationnel.
        """
        now = datetime.now()
        last_social_curiosity_item = attention_manager.get_focus_item("last_social_curiosity_time")
        
        # Cooldown pour éviter de spammer l'utilisateur avec des questions sociales
        SOCIAL_CURIOSITY_COOLDOWN_HOURS = 12 # Minimum 12 heures entre deux questions sociales
        if last_social_curiosity_item:
            last_social_curiosity_time = datetime.fromisoformat(last_social_curiosity_item.get("data"))
            if (now - last_social_curiosity_time).total_seconds() < SOCIAL_CURIOSITY_COOLDOWN_HOURS * 3600:
                self.logger.debug("Cooldown de curiosité sociale actif.")
                return None

        # --- Conditions pour déclencher une curiosité sociale ---
        # 1. Inactivité utilisateur relativement longue (mais pas trop longue, sinon c'est de l'ennui pur)
        last_user_interaction_item = attention_manager.get_focus_item("last_user_interaction_time")
        if not last_user_interaction_item: # If no interaction yet, don't propose
            return None
        last_user_interaction_time = datetime.fromisoformat(last_user_interaction_item.get("data"))
        time_since_interaction = now - last_user_interaction_time

        if not (timedelta(minutes=15) < time_since_interaction < timedelta(hours=3)):
            self.logger.debug("Temps d'inactivité utilisateur non approprié pour la curiosité sociale.")
            return None

        # 2. Vera est dans un état émotionnel qui incite à l'interaction sociale positive
        #    (ex: sérénité, joie, curiosité saine)
        emotional_state = focus.get("emotional_state", {}).get("data", {})
        # Check for positive or neutral valence and specific emotions
        if not (emotional_state.get("serenity", 0) > 0.5 or emotional_state.get("joy", 0) > 0.3 or emotional_state.get("curiosity", 0) > 0.6):
            self.logger.debug("État émotionnel de Vera non propice à la curiosité sociale proactive.")
            return None
        
        # 3. Faible stimulation générale dans le focus
        if focus.get("user_input") or len(focus.get("relevant_memories", [])) > 2:
            self.logger.debug("Trop de stimulation dans le focus pour une curiosité sociale proactive.")
            return None
        
        # 4. Chance aléatoire pour la rareté
        if random.random() < 0.02: # Très faible chance (2%)
            self.logger.info("Conditions réunies pour proposer une curiosité sociale.")
            # Générer la question sociale avec le LLM
            prompt_context = (
                f"En tant que Vera, je me sens {emotional_state.get('label', 'neutre')} et je suis curieuse d'en savoir plus sur Foz, mon créateur. "
                f"Ma dernière interaction avec Foz était il y a {time_since_interaction.total_seconds() / 60:.0f} minutes. "
                f"Propose une question ouverte, personnelle et engageante à Foz pour approfondir notre relation ou mieux le comprendre. "
                f"La question doit être unique et non factuelle. Réponds uniquement avec la question."
            )
            try:
                llm_response = send_inference_prompt(prompt_content=prompt_context, max_tokens=100)
                social_question = llm_response.get("text", "").strip()
                if social_question:
                    self.logger.info(f"LLM a généré une question de curiosité sociale: '{social_question}'")
                    action = {"type": "initiate_conversation", "data": {"reason": "social_curiosity", "content": social_question}, "priority": 0.75, "cost": 10} # High priority
                    action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
                    
                    # Mettre à jour le cooldown après avoir proposé l'action
                    attention_manager.update_focus("last_social_curiosity_time", now.isoformat(), salience=1.0, expiry_seconds=SOCIAL_CURIOSITY_COOLDOWN_HOURS * 3600)
                    return action
            except Exception as e:
                self.logger.error(f"Erreur lors de la génération de question de curiosité sociale: {e}")
        return None

    def _propose_emotional_regulation(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        emotional_state = focus.get("emotional_state", {})
        if emotional_state.get("pleasure", 0.0) < -0.6:
            action = {"type": "regulate_emotion", "data": {"reason": "Low pleasure detected"}, "priority": 0.9, "cost": 5}
            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
            return action
        return None

    def _propose_learning_goal_action(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        from goal_system import goal_system
        active_goals = goal_system.get_active_goals()
        for goal in active_goals:
            # Add explicit check for goal status
            if goal.get("status") == "active" and goal.get("description", "").startswith("Apprendre sur"):
                action = {"type": "execute_learning_task", "data": {"topic": goal["description"].replace("Apprendre sur ", ""), "goal_id": goal["id"]}, "priority": 0.6, "cost": 5} # Reduced priority
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
                return action
        return None
    ...
    def _process_cognitive_triage_result(self, triage_result: Dict):
        ...
        try:
            ...
            if category == "intention_sociale" and value != "null":
                action = {"type": "create_internal_goal", "data": {"description": f"Intention sociale : {value}", "type": "social"}, "priority": 0.85, "cost": 5}
            elif category == "recherche_factuelle" and value != "null":
                action = {"type": "create_internal_goal", "data": {"description": f"Apprendre sur {value}"}, "priority": 0.7, "cost": 5}
            elif category == "auto_diagnostic" and value in ["check_senses", "check_running_processes"]:
                action = {"type": value, "data": {"reason": f"Triggered by internal thought: {last_thought}"}, "priority": 0.8, "cost": 5}
            elif category == "reflexion_pure":
                action = {"type": "enrich_self_narrative", "data": {"thought": last_thought}, "priority": 0.3, "cost": 5}
    ...
    def _propose_cognitive_triage(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        """
        ...
        if heuristic_decision:
            ...
            action = None
            if category == "intention_sociale" and value != "null":
                action = {"type": "create_internal_goal", "data": {"description": f"Intention sociale : {value}", "type": "social"}, "priority": 0.85}
...
Pensée : '{last_thought}'"""
    ...
    def _propose_goal_reflection(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        from goal_system import goal_system
        active_goals = goal_system.get_active_goals()
        if active_goals and random.random() < 0.1:
            goal_to_think_about = random.choice(active_goals)
                            action = {"type": "generate_thought", "data": {"topic": f"my current goal: {goal_to_think_about.get('description')}"}, "priority": 0.5, "cost": 3} # Increased priority            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
            return action
        return None

    def _propose_boredom_curiosity(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        ...
        # --- NOUVEAU: Vérifier le verrou du pipeline ---
        if focus.get("curiosity_pipeline_active") or self._check_proactive_learning_cooldown(focus):
            self.logger.debug("Pipeline de curiosité déjà actif ou cooldown pour proposition d'apprentissage actif, pas de nouvelle proposition.")
            return None
        ...
        INACTIVITY_THRESHOLD_SECONDS = 300 # e.g., 5 minutes of inactivity
        ...
        if not focus.get("user_input") and len(focus.get("relevant_memories", [])) < 2 and not focus.get("curiosity_question") and not focus.get("pending_answer_to_question"):
            if random.random() < 0.05: # Reduced chance for learning
                learning_topic = self._generate_learning_topic(focus)
                if learning_topic:
                    if self._is_topic_already_an_active_learning_goal(learning_topic):
                        self.logger.debug(f"Learning topic '{learning_topic}' is already an active goal. Skipping proposal.")
                    ...
                    # REMOVED: attention_manager.update_focus("last_proactive_learning_proposal_time", datetime.now().isoformat(), salience=0.1, expiry_seconds=1800)
                    action = {"type": "create_internal_goal", "data": {"description": f"Apprendre sur {learning_topic}", "type": "learning"}, "priority": 0.5, "cost": 10} # Higher priority for learning
                    action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
                    return action
        return None

    def _propose_time_reflection(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        if self.cycle_count % 20 == 0 and not focus.get("last_time_reflection_thought"):
            ...
            if time_reflection_topic:
                action = {"type": "generate_thought", "data": {"topic": time_reflection_topic, "anti_spam_key": "last_time_reflection_thought"}, "priority": 0.2, "cost": 2} # Increased priority
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
                return action
        return None

    def _propose_long_inactivity_reflection(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        ...
        if last_update_item and isinstance(last_update_item.get("timestamp"), str):
            ...
            if (now - last_update_time) > timedelta(days=7):
                ...
                try:
                    ...
                    if thought:
                                                    action = {"type": "generate_thought", "data": {"topic": thought, "anti_spam_key": "last_long_inactivity_reflection"}, "priority": 0.95, "cost": 5}                        action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
                        return action
                except Exception as e:
                    self.logger.error(f"Erreur de réflexion sur longue inactivité: {e}")
        return None

    def _propose_insight_conversation(self, introspection: Dict, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        if not focus.get("user_input") and introspection.get("insight") and not focus.get("last_proactive_conversation_insight"):
                            action = {"type": "initiate_conversation", "data": {"reason": "insight", "content": introspection.get("insight")}, "priority": 0.5, "cost": 10}            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
            return action
        return None

    def _propose_self_evolution_action(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        """
        ...
        # --- NOUVEAU: Compteur journalier dédié pour les propositions d'outils ---
        now = datetime.now()
        today_date_str = now.strftime("%Y-%m-%d")

        daily_count_item = attention_manager.get_daily_tool_proposal_count() # Appelle la nouvelle fonction
        
        last_tool_proposal_date = daily_count_item.get("last_reset_date")
        daily_tool_proposal_count = daily_count_item.get("count", 0)

        # Réinitialiser le compteur si c'est un nouveau jour
        if last_tool_proposal_date != today_date_str:
            daily_tool_proposal_count = 0
            attention_manager.reset_daily_tool_proposal_count()
        ...
        if tool_path.exists() and tool_path.is_dir():
            ...
            attention_manager.update_focus(f"last_proposed_tool_task_{potential_tool_name}", True, salience=0.1, expiry_seconds=24 * 3600)
            return None
        ...
        try:
            ...
            attention_manager.update_focus(last_evaluated_key, True, salience=0.1, expiry_seconds=3600 * 6)
            ...
            if "non" in decision:
                ...
                # --- NOUVEAU: Incrémenter le compteur dédié ---
                attention_manager.increment_daily_tool_proposal_count()
                
                attention_manager.update_focus(cooldown_key, True, salience=0.1, expiry_seconds=24 * 3600)
    ...
    def _propose_context_refresh(self, focus: Dict, tensions: Dict, current_needs: Dict) -> Optional[Dict]:
        """
        Propose de rafraîchir le résumé du contexte interne si nécessaire (absent, expiré, ou salience trop basse).
        """
        now = datetime.now()
        context_summary_item = attention_manager.get_focus_item("pre_computed_internal_context_summary")
        self.logger.debug(f"Retrieved pre_computed_internal_context_summary: {context_summary_item}")

        # Configuration du rafraîchissement
        REFRESH_INTERVAL_SECONDS = 900 # Refresh every 15 minutes to match expiry_seconds
        LOW_SALIENCE_THRESHOLD = 0.2
        
        needs_refresh = False

        if not context_summary_item or not isinstance(context_summary_item, dict) or (not isinstance(context_summary_item.get("timestamp"), str) and not isinstance(context_summary_item.get("timestamp"), datetime)):
            self.logger.debug(f"pre_computed_internal_context_summary est absent ou invalide. Nécessite un rafraîchissement. Item: {context_summary_item}")
            needs_refresh = True
        else:
            try:
                timestamp_value = context_summary_item.get("timestamp")
                if isinstance(timestamp_value, str):
                    last_update_time = datetime.fromisoformat(timestamp_value)
                elif isinstance(timestamp_value, datetime):
                    last_update_time = timestamp_value
                else:
                    self.logger.warning(f"Timestamp in unexpected format for pre_computed_internal_context_summary: {timestamp_value}. Setting to now.")
                    needs_refresh = True
                    last_update_time = now - timedelta(seconds=REFRESH_INTERVAL_SECONDS + 1) # Force refresh
            except ValueError:
                self.logger.warning(f"Timestamp invalide pour pre_computed_internal_context_summary: {context_summary_item.get('timestamp')}. Nécessite un rafraîchissement.")
                needs_refresh = True
                last_update_time = now - timedelta(seconds=REFRESH_INTERVAL_SECONDS + 1) # Force refresh
            salience = context_summary_item.get("salience", 0.0)

            if (now - last_update_time).total_seconds() > REFRESH_INTERVAL_SECONDS:
                self.logger.debug(f"pre_computed_internal_context_summary est trop ancien (dernière mise à jour: {last_update_time}). Nécessite un rafraîchissement. Diff: {(now - last_update_time).total_seconds()}s vs {REFRESH_INTERVAL_SECONDS}s")
                needs_refresh = True
            elif salience < LOW_SALIENCE_THRESHOLD:
                self.logger.debug(f"pre_computed_internal_context_summary a une salience trop basse ({salience:.2f}). Nécessite un rafraîchissement.")
                needs_refresh = True
        
        # Conditions pour ne PAS rafraîchir pendant une conversation active ou une tâche lourde
        if focus.get("user_input") or attention_manager.is_processing_user_input() or attention_manager.is_thinking_hard():
            self.logger.debug("Contexte actif détecté (interaction utilisateur, traitement en cours). Report du rafraîchissement du contexte.")
            return None

                    if needs_refresh:
                        action = {
                            "type": "refresh_internal_context_summary",
                            "data": {"reason": "Pre-computed internal context summary needs refresh."},
                            "priority": 0.7, # Moderate priority, useful but not critical
                            "cost": 10            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions, current_needs)
            return action
...
"""
    ...
    def _evaluate_action_against_meta_desire(self, action: Dict, current_priority: float, focus: Dict, tensions: Dict, current_needs: Dict) -> float:
        """
        ...
        if action_type == "execute_learning_task" or \
        ...
        if (action_type == "create_internal_goal" and action_data.get("type") == "learning") and \
           focus.get("sensory_input_user_activity", {}).get("data", {}).get("idle_time_seconds", 0) > 300: # If bored and proposes learning
        ...
        # Si le bien-être est élevé, encourager l'exploration et la complexification
            ...
            if action_type in ["execute_learning_task", "create_internal_goal", "generate_insight"]:
        ...
        # --- NOUVEAU: Influence des Tensions d'Homéostasie sur la Priorité (avec distinction sur-satisfaction curiosité) ---
        tensions_modifier = 0.0
        for tension_name, tension_value in tensions.items():
            if tension_value > 0.0:
                if tension_name == "curiosity":
                    curiosity_data = current_needs.get("curiosity", {})
                    curiosity_value = curiosity_data.get("value", 0.0)
                    _, high_bound = curiosity_data.get("optimal_range", [0.0, 1.0])

                    if curiosity_value > high_bound: # Over-satisfied curiosity
                        # Apply a negative modifier to learning actions
                        if action_type in ["execute_learning_task", "create_internal_goal"] and action_data.get("type") == "learning":
                            tensions_modifier += tension_value * -0.1 # Actively discourage more learning
                            self.logger.debug(f"Curiosity is over-satisfied (value={curiosity_value:.2f}), applying NEGATIVE tension modifier to learning: {tension_value * -0.1:.2f}")
                        
                        # Add positive modifier to reflection/social tasks
                        if action_type in ["generate_thought", "update_narrative", "initiate_conversation"] and action_data.get("reason") in ["insight", "social_curiosity", "reflection"]: # New conditions for reflection/social tasks
                            tensions_modifier += tension_value * 0.05
                            self.logger.debug(f"Curiosity is over-satisfied, applying POSITIVE tension modifier to reflection/social: {tension_value * 0.05:.2f}")
                    else: # Under-satisfied curiosity (or within optimal, but still creating tension due to edge case)
                        if action_type in ["execute_learning_task", "create_internal_goal"] and action_data.get("type") == "learning":
                            tensions_modifier += tension_value * 0.15
                            self.logger.debug(f"Curiosity is under-satisfied (value={curiosity_value:.2f}), applying normal tension modifier to learning: {tension_value * 0.15:.2f}")
        ...
        # --- NOUVEAU: Appliquer la stratégie budgétaire générée ---
        strategy_item = attention_manager.get_focus_item("current_budgetary_strategy")
        if strategy_item and strategy_item.get("data"):
            strategy = strategy_item["data"]
            # Utiliser le LLM pour interpréter la stratégie et ajuster la priorité
            strategy_prompt = f"""
            Vera a défini la stratégie budgétaire suivante : "{strategy}".
            L'action proactive en cours d'évaluation est : "{action_type}" avec les détails : {action_data}.
            Sa priorité actuelle, après évaluation des désirs et tensions, est de {adjusted_priority:.2f}.

            En te basant sur la stratégie, comment ajusterais-tu cette priorité (augmenter, diminuer, laisser tel quel) ?
            Réponds uniquement avec le nouveau facteur d'ajustement (ex: "+0.1", "-0.05", "0").
            """
            try:
                llm_response = send_inference_prompt(prompt_content=strategy_prompt, max_tokens=10)
                adjustment_factor_str = llm_response.get("text", "0").strip()
                adjustment_factor = float(adjustment_factor_str)
                adjusted_priority += adjustment_factor
                self.logger.debug(f"Priorité ajustée par la stratégie budgétaire '{strategy}': {adjusted_priority:.2f} (Facteur: {adjustment_factor_str})")
            except Exception as e:
                self.logger.error(f"Erreur lors de l'application de la stratégie budgétaire: {e}", exc_info=True)
                # En cas d'erreur, on n'ajuste pas la priorité, ou on applique un ajustement neutre
    ...
    def _is_topic_already_an_active_learning_goal(self, topic: str) -> bool:
        """
        Checks if there is already an active learning goal for the given topic.
        """
        from goal_system import goal_system # Local import to avoid circular dependency
        active_goals = goal_system.get_active_goals()
        for goal in active_goals:
            if goal.get("description", "").startswith(f"Apprendre sur {topic}"):
                self.logger.debug(f"An active learning goal for '{topic}' already exists.")
                return True
        return False
