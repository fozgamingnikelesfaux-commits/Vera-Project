    ...
    20}}"""
        ...
        if pending_user_command_item:
            ...
            if "approbation" in user_intent:
                ...
                Contexte interne de Vera au moment de poser la question (pour comprendre son état d'esprit) : {pending_question_data.get('context_when_asked')}
        ...
        if task_type == "process_user_input_task":
            ...
            try:
                ...
                with metacognition.lock: # Ensure thread-safe update
                    # Assuming insight is stored in a sub-section of metacognition.state['learning']['last_insights']
                    # This needs to be done within metacognition's update logic, not directly here.
                    # For now, just save the overall state correctly.
            ...
            attention_manager.update_focus("pending_answer_to_question", {
                ...
                "reason_asked": reason,
                "context_when_asked": current_focus # Store the actual focus at the time of asking
            ...
            for action_name in actions_to_perform:
                ...
                if action_name in critical_danger_actions:
                    ...
                    llm_refusal_message = send_inference_prompt(prompt_content=prompt, max_tokens=200).get("text", "Je ne peux pas faire cela, Foz. C'est trop dangereux pour notre monde.").strip()
                    memory_manager.ajouter_evenement(
                        llm_refusal_message,
            ...
            memory_manager.ajouter_evenement(
                ...
                emotion=_get_current_emotion(),
                intention=_infer_intention(["vera_response", "action_report"], final_report_message),
                attention_focus=_get_current_attention_focus()
            )
...
def _queue_approved_actions_execution(actions: List[str]):
    """
    Adds a task to the slow path queue to execute a list of approved actions.
    """
    slow_path_task_queue.put((1, next(task_counter), { # Priority 1 for user-approved actions
...
def process_user_input(user_input: str, image_path: Optional[str] = None):
    ...
    if pending_user_command_item and pending_user_command_item.get("data", {}).get("type") == "system_cleanup":
        ...
        if "approbation" in user_intent:
            ...
            # Instead of executing here, enqueue the task for the Slow Path
            _queue_approved_actions_execution(pending_action.get("actions", []))
