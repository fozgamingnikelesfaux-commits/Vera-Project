...
class MetaCognition:
    ...
    def decide_proactive_action(self, introspection: Dict, focus: Dict) -> Optional[Dict]:
        ...
        winning_bid = max(valid_bids, key=lambda x: x['priority'])
        
        self.logger.info(f"Économie Cognitive: Gagnant: {winning_bid['type']} (Priorité: {winning_bid['priority']:.2f})")
    ...
    def _propose_cleanup_suggestions(self, focus: Dict, tensions: Dict) -> Optional[Dict]:
        ...
        if disk_c_free is not None:
            if disk_c_free < 5 and not focus.get("last_proactive_suggestion_winsxs_cleanup"):
                action = {"type": "suggest_system_cleanup", "data": {"reason": f"L'espace disque sur C: est critique ({disk_c_free:.2f} Go restants).", "actions": ["cleanup_winsxs", "uninstall_superseded_updates"]}, "priority": 0.85, "spam_flag": "last_proactive_suggestion_winsxs_cleanup"}
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
                return action
            if disk_c_free < 10 and not focus.get("last_proactive_suggestion_winupdate_cleanup"):
                action = {"type": "suggest_system_cleanup", "data": {"reason": f"L'espace disque sur C: est très faible ({disk_c_free:.2f} Go restants).", "actions": ["clear_windows_update_cache"]}, "priority": 0.80, "spam_flag": "last_proactive_suggestion_winupdate_cleanup"}
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
                return action
            if disk_c_free < 20 and not focus.get("last_proactive_suggestion_disk_cleanup"):
                action = {"type": "suggest_system_cleanup", "data": {"reason": f"L'espace disque sur C: est faible ({disk_c_free:.2f} Go restants).", "actions": ["clear_windows_temp", "clear_user_temp", "clear_thumbnail_cache", "empty_recycle_bin"]}, "priority": 0.75, "spam_flag": "last_proactive_suggestion_disk_cleanup"}
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
                return action
        ...
        if 75.0 < ram_usage < 90.0 and not focus.get("last_proactive_suggestion_ram_check"):
            action = {"type": "suggest_check_running_processes", "data": {"reason": f"L'utilisation de la RAM est élevée ({ram_usage:.2f}%)."}, "priority": 0.65, "spam_flag": "last_proactive_suggestion_ram_check"} # Lowered priority
            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
            return action
        return None
    def _propose_desire_based_actions(self, focus: Dict, tensions: Dict) -> Optional[Dict]:
        ...
        if "Prendre soin de Foz (suggérer une pause)" in active_desires and not focus.get("last_proactive_suggestion_foz_break"):
            action = {"type": "initiate_conversation", "data": {"reason": "care_for_foz", "content": "Foz, j'ai l'impression que tu travailles beaucoup. Peut-être devrais-tu faire une petite pause ?"}, "priority": 0.9, "spam_flag": "last_proactive_suggestion_foz_break"}
            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
            return action
        
        if "Partager ma joie ou mon enthousiasme" in active_desires and not focus.get("last_proactive_share_joy"):
            action = {"type": "initiate_conversation", "data": {"reason": "share_joy", "content": "Je me sens particulièrement joyeuse en ce moment ! Y a-t-il quelque chose que tu aimerais que nous fassions ensemble ?"}, "priority": 0.7, "spam_flag": "last_proactive_share_joy"}
            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
            return action
    ...
    def _propose_learning_from_desire(self, focus: Dict, tensions: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            action = {"type": "ask_curiosity_question", "data": {"reason": "desire_to_learn", "content": curiosity_question}, "priority": 0.6, "spam_flag": "last_proactive_desire_to_learn"}
            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
            return action
        except Exception as e:
    ...
    def _propose_emotional_regulation(self, focus: Dict, tensions: Dict) -> Optional[Dict]:
        ...
        if emotional_state.get("pleasure", 0.0) < -0.6:
            action = {"type": "regulate_emotion", "data": {"reason": "Low pleasure detected"}, "priority": 0.9}
            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
            return action
        return None
    ...
    def _propose_learning_goal_action(self, focus: Dict, tensions: Dict) -> Optional[Dict]:
        ...
        for goal in active_goals:
            if goal.get("description", "").startswith("Apprendre sur"):
                action = {"type": "execute_learning_task", "data": {"topic": goal["description"].replace("Apprendre sur ", ""), "goal_id": goal["id"]}, "priority": 0.9}
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
                return action
        return None
    ...
    def _process_cognitive_triage_result(self, triage_result: Dict):
        ...
        try:
            ...
            if category == "intention_sociale" and value != "null":
                ...
                action = {"type": "create_internal_goal", "data": {"description": f"Apprendre sur {value}"}, "priority": 0.7}
            elif category == "auto_diagnostic" and value in ["check_senses", "check_running_processes"]:
                action = {"type": value, "data": {"reason": f"Triggered by internal thought: {last_thought}"}, "priority": 0.8}
            elif category == "reflexion_pure":
                action = {"type": "enrich_self_narrative", "data": {"thought": last_thought}, "priority": 0.3}
...
Pensée : '{last_thought}'"""
    ...
    def _propose_goal_reflection(self, focus: Dict, tensions: Dict) -> Optional[Dict]:
        ...
        if active_goals and random.random() < 0.1:
            ...
            action = {"type": "generate_thought", "data": {"topic": f"my current goal: {goal_to_think_about.get('description')}"}, "priority": 0.5} # Increased priority
            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
            return action
        return None
    ...
    def _propose_boredom_curiosity(self, focus: Dict, tensions: Dict) -> Optional[Dict]:
        ...
        if not focus.get("user_input") and len(focus.get("relevant_memories", [])) < 2 and not focus.get("curiosity_question") and not focus.get("pending_answer_to_question"):
            ...
            if random.random() < 0.05: # Reduced chance to 5%
                action = {"type": "ask_curiosity_question", "data": {"reason": "Low stimulation in focus"}, "priority": 0.3} # Increased priority
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
                return action
        return None
    ...
    def _propose_time_reflection(self, focus: Dict, tensions: Dict) -> Optional[Dict]:
        if self.cycle_count % 20 == 0 and not focus.get("last_time_reflection_thought"):
            ...
            if time_reflection_topic:
                action = {"type": "generate_thought", "data": {"topic": time_reflection_topic, "anti_spam_key": "last_time_reflection_thought"}, "priority": 0.2} # Increased priority
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
                return action
        return None
        ...
        if last_update_item and isinstance(last_update_item.get("timestamp"), str):
            ...
            if (now - last_update_time) > timedelta(days=7):
                ...
                try:
                    ...
                    if thought:
                        action = {"type": "generate_thought", "data": {"topic": thought, "anti_spam_key": "last_long_inactivity_reflection"}, "priority": 0.95}
                        action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
                        return action
                except Exception as e:
...
Décide de la destination la plus appropriée. Réponds UNIQUEMENT par 'humain' ou 'internet'."""
            ...
            if decision == "humain":
                if not focus.get("pending_answer_to_question") and not focus.get("last_proactive_conversation_curiosity"):
                    ...
                    action = {"type": "initiate_conversation", "data": {"reason": "curiosity", "content": question, "topic": topic}, "priority": 0.6}
                    action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
                    return action
            else:
                action = {"type": "learn_from_curiosity", "data": {"question": question}, "priority": 0.55}
                action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
                return action
        return None
    def _propose_insight_conversation(self, introspection: Dict, focus: Dict, tensions: Dict) -> Optional[Dict]:
        if not focus.get("user_input") and introspection.get("insight") and not focus.get("last_proactive_conversation_insight"):
            action = {"type": "initiate_conversation", "data": {"reason": "insight", "content": introspection.get("insight")}, "priority": 0.5}
            action["priority"] = self._evaluate_action_against_meta_desire(action, action["priority"], focus, tensions)
            return action
        return None
