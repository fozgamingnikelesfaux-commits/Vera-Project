...
def _format_structured_memories_for_llm(limit: int = 5) -> str:
    """
    Fetches recent structured memories and formats them into a human-readable
    narrative string for the LLM prompt.
    """
    from episodic_memory import memory_manager
    from datetime import datetime

    recent_events = memory_manager.get_recent(limit=limit)
    if not recent_events:
        return ""

    narrative_parts = ["(Voici un résumé de mes expériences les plus récentes pour contextualiser ma pensée :"]

    for event in reversed(recent_events): # Reverse to have newest first
        try:
            event_data = event.get('context', {}) # The full data is in 'context'
            if not event_data:
                continue
            ...
            if time_delta.total_seconds() < 60:
                time_ago = f"{int(time_delta.total_seconds())} secondes"
            else:
                time_ago = f"{int(time_delta.total_seconds() / 60)} minutes"

            initiator = event_data.get('initiator', 'unknown')
            description = event_data.get('description', '')
            snapshot = event_data.get('snapshot', {})
            emotion_state = snapshot.get('emotional_state', {})
            
            # Summarize emotion
            active_emotions = {k: v for k, v in emotion_state.items() if isinstance(v, (int, float)) and v > 0.2}
            sorted_emotions = sorted(active_emotions.items(), key=lambda item: item[1], reverse=True)
            emotion_summary = sorted_emotions[0][0] if sorted_emotions else "neutre"

            narrative_line = ""
            if event.get('type') == 'user_interaction':
                narrative_line = f"- Il y a {time_ago}, Foz a dit: '{description[:80]}...'. À ce moment, je me sentais {emotion_summary}."
            elif event.get('type') == 'proactive_action':
                reason = event_data.get('reason', 'une impulsion interne')
                narrative_line = f"- Il y a {time_ago}, j'ai décidé de '{description[:80]}...' (cause: {reason}). Je me sentais {emotion_summary}."
            
            if narrative_line:
                narrative_parts.append(narrative_line)

        except Exception as e:
            logger.warning(f"Could not format structured memory event: {e}")
            continue
...
def _threaded_generate_response(queue: Queue, user_input: str, attention_focus: Dict[str, Any], internal_state: Dict, image_path: Optional[str] = None):
    ...
    try:
        ...
        # --- Add rich, narrative summary of recent experiences ---
        rich_memory_narrative = _format_structured_memories_for_llm()
        if rich_memory_narrative:
            prompt_parts.append(rich_memory_narrative)
